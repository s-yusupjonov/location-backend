...
public class DepartmentController {
    ...
    public List<DepartmentResponse> getAll() {
        return departmentService.findAll().stream().map(departmentMapper::toResponse).toList();
    }
    ...
    public ResponseEntity<DepartmentResponse> create(@Valid @RequestBody DepartmentRequest request) {
        DepartmentResponse response = departmentMapper.toResponse(departmentService.create(request));
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @PutMapping("/{id}")
    public DepartmentResponse update(@PathVariable Long id, @Valid @RequestBody DepartmentRequest request) {
        return departmentMapper.toResponse(departmentService.update(id, request));
    }
